def select_books_titles_and_years_in_first_series_order_by_year
  "SELECT books.title, year FROM books JOIN series ON series.id = books.series_id WHERE series.id = 1 ORDER BY year"
end
...
def select_value_and_count_of_most_prolific_species
  "SELECT species, COUNT(species) AS prolific FROM characters GROUP BY species ORDER BY prolific DESC LIMIT 1"
end
...
def select_character_names_and_number_of_books_they_are_in
  "SELECT characters.name, COUNT(books.id) AS num FROM characters
